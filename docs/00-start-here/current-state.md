...
- **Post-merge stabilization (2026-07-18)** — Fixed four confirmed defects: operator confirm cancel no longer executes destructive actions (`confirmResult.confirmed` contract); operator enqueue uses `executionType` matching production `/jobs`; background worker fails jobs when workflow/track execution returns failed or invalid schema (via `companion/jobs/job-outcome.js`); Development Memory server endpoints resolve active-project paths through `companion/memory/projects/memory-services.js`. Canonical non-hardware suite: `npm run test:full`. Production jobs contract test: `npm run test:jobs:production`.
- **Durable Job Store API** — `POST /jobs` expects `{ executionType, trackId|workflowId, input }`. `GET /jobs`, `GET /jobs/:id`, mutation endpoints (`cancel`, `retry`, `review`). `GET /health` includes `jobTotals`. 64 tests in `scripts/test-jobs-api.js`; 8 production-route tests in `scripts/test-jobs-api-production.js`.
- **Background Worker Polling Loop** — `companion/jobs/worker.js` + `companion/jobs/job-outcome.js`. Fails jobs on workflow `plan.status=failed`, `schemaValid=false`, or track verification failures — not only thrown exceptions. 50 tests in `scripts/test-jobs-worker.js`.
...
- **Smoke and contract tests** - `scripts/smoke-test.js` (58/58), `scripts/contract-test.js`, canonical offline suite `npm run test:full`
...
- **LAN Security Hard Gate** - non-loopback binding requires `RELAY_TOKEN`, `LAN_MODE=1`, `RELAY_ALLOWLIST`, and `RELAY_CAPABILITY_ALLOWLIST`; server refuses startup otherwise. All relay endpoints enforce Bearer token authentication when `RELAY_TOKEN` is configured. 20/20 security gate tests pass.
...
- **Relay trust boundary** — API-level Bearer token authentication exists (all relay endpoints require auth when `RELAY_TOKEN` is set); node-to-node trust boundary (pairing, signed requests, credential rotation) remains unimplemented — current design is trusted-development-network only for inter-node trust
