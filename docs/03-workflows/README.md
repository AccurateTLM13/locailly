# Workflows

User-facing jobs built from tracks. Workflows compose core and specialized track steps.

| Workflow | Status | Doc |
|---|---|---|
| Lighthouse Handoff | **Implemented** (proof track) | [lighthouse-handoff.md](./lighthouse-handoff.md) |
| DealSniper | Partial (tool only) | [dealsniper.md](./dealsniper.md) |
| Content OS | Planned | — |
| Repo Review | Planned | — |

## Templates

- [workflow-template.md](./workflow-template.md) — document a new workflow
- [task-track-template.md](./task-track-template.md) — Pit Crew track JSON template
- [validation-template.md](./validation-template.md) — validation evidence template

## Related

- Track composition: [../02-track-system/workflow-registry.md](../02-track-system/workflow-registry.md)
- Validation evidence: [../04-validation/README.md](../04-validation/README.md)
