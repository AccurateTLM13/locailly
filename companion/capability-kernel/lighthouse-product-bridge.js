...
function convertPagespeedToMarkdown(pagespeedData, options = {}) {
  const url = pagespeedData.url || pagespeedData.requestedUrl || "https://example.com";
  const scores = pagespeedData.scores || { performance: 85, accessibility: 92, seo: 95, best_practices: 90 };
  ...
  for (const issue of issues) {
    ...
    lines.push(`- ${icon} **[${issue.category.toUpperCase()}]** ${issue.title}`);
  }

  return lines.join("\n");
}

function enhanceReportWithLocalAI(reportMarkdown, aiSummary = null) {
