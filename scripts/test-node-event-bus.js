...
try {
  ...
  runTest("AC 1: Event bus supports topic pattern matching with wildcards (*, #)", () => {
  ...
  runTest("AC 2: Pub/Sub event bus dispatches events to registered subscribers via LocalTransport", async () => {
  ...
  runTest("AC 3: Event bus records complete delivery provenance (delivered_to, latency_ms)", async () => {
    const bus = createEventBus();
  ...
  runTest("AC 4: Subscriber execution errors are caught without crashing event bus", async () => {
    const bus = createEventBus();
