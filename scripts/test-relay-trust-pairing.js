...
try {
  ...
  runTest("AC 1: Nodes complete 4-step challenge-response pairing ceremony and acquire trust tokens", () => {
  ...
  runTest("AC 2: Inter-node requests are signed with HMAC-SHA256 and verified against node secret tokens", () => {
  ...
  // AC 3: Untrusted Node Rejection
  runTest("AC 3: Requests to or from untrusted, unpaired nodes are rejected with UNTRUSTED_NODE", async () => {
    const hostNode = normalizeNodeConfig({ node_id: "node-brain-host" });
  ...
  runTest("AC 4: Revoked nodes are immediately rejected from inter-node execution", async () => {
    const hostNode = normalizeNodeConfig({ node_id: "node-brain-host" });
